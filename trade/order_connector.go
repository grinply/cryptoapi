...
type OrderConnector interface {
	...
	OpenOrder(tradingPair CurrencyPair) (string, error)
	...
	// CancelAllOrders cancels all open orders that were previously placed in the exchange orderbook for the provided trading pair.
	...
	CancelAllOrders(tradingPair CurrencyPair) error
}
